...
async function fetchListings(params) {
  const body = {
    json_schema: {
      category: params.category,      // مثال: 'apartment-rent'
      city: params.city,              // مثال: 'mashhad'
      query: params.query || '',
      sort: { sort_by: 'date' },
    },
  };
  ...
  setInterval(async () => {
    try {
      ...
      for (const p of posts) {
        ...
        // اینجا هندل حرفه‌ای:
        // ارسال به تلگرام / ذخیره DB / لاگ
        console.log(`[NEW] ${p.data?.title} | ${p.data?.description} | ${p.data?.token}`);
      }

      // محدود کردن سایز حافظه
